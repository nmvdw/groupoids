...
Section LaxLeftIdentityInv.
  Context `{Univalence}
          {C D : BiCategory}.
  Variable (F : LaxFunctor C D).
  
  Definition lax_left_identity_inv_d
    : LaxTransformation_d F (lax_comp (lax_id_functor D) F).
  Proof.
    ...
    - intros X Y ; cbn.
      ...
      refine (whisker_r
                (inverse (un_l (F X) (F Y)) o un_r (F X) (F Y))
                (Fmor F)
                o whisker_l _ _)%natural_transformation.
      apply Composition.right_identity_natural_transformation_1.
  Defined.

  Definition is_lax_lax_left_inv_d
    : is_lax_transformation lax_left_identity_inv_d.
  Proof.
    split.
    - intros X ; cbn.
  ...
  Definition lax_left_identity_inv
    : LaxTransformation F (lax_comp (lax_id_functor D) F)
    := Build_LaxTransformation lax_left_identity_inv_d is_lax_lax_left_inv_d.

  Global Instance left_identity_inv_pseudo
    : is_pseudo_transformation lax_left_identity_inv.
  Proof.
    simple refine {| laxnaturality_of_iso := _ |}.
  Defined.
End LaxLeftIdentityInv.